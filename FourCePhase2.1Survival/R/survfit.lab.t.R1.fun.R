...
survfit.lab.t.R1.fun=function(dat.survival, LocalPatientObservations, nm.event, dir.output, nm.lab.keep, t0.all, rm.event.baseline=F, is.bt=T){
  ...
  patient.rm=dat0[which(dat0$days_since_admission==0 & dat0[,nm.event]==1),"patient_num"]
  ...
  roc.tmp=tryCatch(ROC.Est.FUN(dat.tmp[,nm.event], dat.tmp[,nm.lab], yy0=0.5, fpr=seq(0,1,0.01)),error=function(e) NA)
  if(is.na(roc.tmp[1])!=1){
  if(roc.tmp[1]<0.5){roc.tmp=ROC.Est.FUN(1-dat.tmp[,nm.event], dat.tmp[,nm.lab], yy0=0.5, fpr=seq(0,1,0.01))}
  ...
  if(is.bt==T){roc.tmp.bt=lapply(1:100, function(myseed){
    ...
    dat.bt=dat.tmp[sample(1:dim(dat.tmp)[1], dim(dat.tmp)[1],replace=T), ]
      res=tryCatch(ROC.Est.FUN(dat.bt[,nm.event], dat.bt[,nm.lab], yy0=0.5, fpr=seq(0,1,0.01))[1],error=function(e) NA)
      res
  })}else{roc.tmp.bt=NA}
