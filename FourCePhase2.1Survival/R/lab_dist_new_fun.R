lab_dist_new_fun=function(dir.input, before.14=T, code.dict, LocalPatientObservations, dat.survival,calendar.date.cut="2020-07", myscale){
...
if(nm.event=="all"){
tmp=data_lab_clean2(code.dict, dat.x.raw, 
                            nm.value="value", day=myday) 
}
  
if(nm.event=="severedeceased"){
tmp=data_lab_clean2(code.dict, dat.x.raw[dat.x.raw$patient_num%in%patient_num.severedeceased,], 
                        nm.value="value", day=myday) 
}
  
if(nm.event=="deceased"){
tmp=data_lab_clean2(code.dict, dat.x.raw[dat.x.raw$patient_num%in%patient_num.deceased,], 
                        nm.value="value", day=myday) 
}
