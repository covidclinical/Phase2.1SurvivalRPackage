lab_recoverRate_new_strat_rmDead_fun=function(dir.input, strat="max_day", dat.cls, pat.days.cut1=0, pat.days.cut2=14, days.range=c(1:14), code.dict, nm.lab.keep,LocalPatientObservations,LocalPatientClinicalCourse, dat.survival,calendar.date.cut, myscale, is.ns=0){
...
dat.dead=dat.cc[dat.cc$deceased==1,]
dat.dead=dat.dead[,c("patient_num", "days_since_admission")]
dat.dead=data.table(dat.dead)
dat.dead=data.frame(dat.dead[,lapply(.SD, min), by=patient_num])

resN=NULL
for(ii in names(day.range.list)){
  myrange=day.range.list[[ii]]
  patient.dead.rm=dat.dead[dat.dead$days_since_admission%in%myrange, "patient_num"]
  junk1=dat.calendar.early[(dat.calendar.early$max_day%in%myrange) & (dat.calendar.early$patient_num%in%patient.dead.rm!=1),]
  junk2=dat.calendar.late[(dat.calendar.late$max_day%in%myrange) & (dat.calendar.late$patient_num%in%patient.dead.rm!=1),]
  n.early=dim(junk1)[1]
  n.late=dim(junk2)[1]
...
for(nm.lab in nm.lab.keep){
 ...
 if(dim(dat.lab)[1]>10){
  ...
  myknots=myknots[myknots<=max(dat.lab[dat.lab$calendar_date<calendar.date.cut& dat.lab[,strat]%in%myrange & (dat.lab$patient_num%in%patient.dead.rm!=1),"day"])]
  m1.early<- tryCatch(lmer(y ~ ns(day,knots=myknots)+charlson_score+(day|patient_num), data=dat.lab[dat.lab$calendar_date<calendar.date.cut& dat.lab[,strat]%in%myrange & (dat.lab$patient_num%in%patient.dead.rm!=1),]),error=function(e) NA)
  m1.late <- tryCatch(lmer(y ~ ns(day,knots=myknots)+charlson_score+(day|patient_num), data=dat.lab[dat.lab$calendar_date>=calendar.date.cut& dat.lab[,strat]%in%myrange & (dat.lab$patient_num%in%patient.dead.rm!=1),]),error=function(e) NA)

  ff=ns(c(days.range,dat.lab[dat.lab$calendar_date<calendar.date.cut& dat.lab[,strat]%in%myrange & (dat.lab$patient_num%in%patient.dead.rm!=1),]$day),knots=myknots)[1:length(days.range),]
